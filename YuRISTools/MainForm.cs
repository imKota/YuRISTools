...
namespace YuRISTools
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ypf_unpack_input.Text));
            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ypf_unpack_output.Text));

            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ystb_text_patched_done.Text));

            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ystb_text_input.Text));
            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ystb_text_output.Text));

            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ystb_text_patched_input.Text));
            Directory.CreateDirectory(Path.GetDirectoryName(this.textBox_ystb_text_patched_done.Text));
        ...
        private void button_ystb_cipher_decrypt_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private void button_ystb_cipher_encrypt_Click(object sender, EventArgs e)
        {
            try
            {
                var key = int.Parse(textBox_ystb_cipher_key.Text, NumberStyles.HexNumber);
                var files = new List<string>();
                int counter = 0;
                if (Directory.Exists(textBox_ystb_text_patched_input.Text))
                {
                    files.AddRange(Directory.GetFiles(textBox_ystb_text_patched_input.Text, "*.ybn", SearchOption.TopDirectoryOnly));
                }
                else
                {
                    files.Add(textBox_ystb_text_patched_input.Text);
                }
                foreach (var file in files)
                {
                    using (var reader = new BinaryReader(File.OpenRead(file)))
                    {
                        ...
                        if (data != null)
                        {
                            reader.Close();
                            Log("[YSTB Cipher] Processed: " + Path.GetFileName(file));
                            var path = Path.Combine(textBox_ystb_text_patched_done.Text, Path.GetFileName(file));
                            Directory.CreateDirectory(Path.GetDirectoryName(path));
                            File.WriteAllBytes(path, data);
                            counter++;
                        }
